...
static void msdc_set_bad_card_and_remove(struct msdc_host *host)
{
    ...
    if(host->mmc->card){
        ...
        else //Modify by zhanying.yang for MS can't power off after plug out the SD card
...
static void msdc_tasklet_card(unsigned long arg)
{
    ...
    if (!host->suspend && host->sd_cd_insert_work) {//Modify by zhanying.yang for MS can't power off after plug out the SD card
        if (inserted) {
...
static u32 msdc_power_tuning(struct msdc_host *host)
{
    ...
    if (mmc_card_sd(card) && (host->hw->host_function == MSDC_SD)) {
        if((host->power_cycle < MSDC_MAX_POWER_CYCLE) && (host->power_cycle_enable))
        {
            ...
            spin_unlock(&host->lock);//Modify by zhanying.yang for MS can't power off after plug out the SD card
            msdc_set_bad_card_and_remove(host);
            spin_lock(&host->lock);//End
        }
    }
    return ret;
}
...
int msdc_pio_read(struct msdc_host *host, struct mmc_data *data)
{
    ...
    while (1) {
        ...
        if(ptr==NULL||(PageHighMem((struct page *)(sg->page_link & ~0x3))))
        {
            ...
            //Modify by zhanying.yang for MS can't power off after plug out the SD card
