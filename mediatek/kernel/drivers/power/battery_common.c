...
{ 
	...
	battery_xlog_printk(BAT_LOG_CRTI, "AvgVbat=(%d),bat_vol=(%d),AvgI=(%d),I=(%d),VChr=(%d),AvgT=(%d),T=(%d),pre_SOC=(%d),SOC=(%d),ZCV=(%d),BatteryID=(%d)\n",
		BMT_status.bat_vol,bat_vol,BMT_status.ICharging,ICharging,BMT_status.charger_vol,BMT_status.temperature,temperature,previous_SOC,BMT_status.SOC,BMT_status.ZCV,BMT_status.id_vol);	
...
#if BATT_ID_CHECK_SUPPORT
#define CHG_MIN_VALID_BATT_ID_BYD  325
#define CHG_MAX_VALID_BATT_ID_BYD  525
...
/*[FR618922][soul45][charge]import new temperature policy by [wangxingxing] at [20140327] begin*/
	if(BMT_status.temperature >= MAX_CHARGE_TEMPERATURE) 
    {
        g_BatteryNotifyCode |= 0x0002;
        battery_xlog_printk(BAT_LOG_CRTI, "[Discharging] bat_temp(%d) out of range(too high)\n", BMT_status.temperature);		
    }
/*[805255][Temperature]MS prompts "the handset has stopped charging!" though without charger. by [bin.song.hz] at [2014.10.14] begin*/    
    if((BMT_status.temperature >= MAX_CHARGE_TEMPERATURE) && (BMT_status.charger_exist == KAL_TRUE))
    {
      g_BatteryNotifyCode |= 0x0020;
       battery_xlog_printk(BAT_LOG_CRTI, "[Charging] bat_temp(%d) out of range(too high)\n", BMT_status.temperature);
    }
#if defined(MTK_JEITA_STANDARD_SUPPORT)
	if (BMT_status.temperature < TEMP_NEG_10_THRESHOLD)
	{
        //g_BatteryNotifyCode |= 0x0020;
...
#else
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
	if (BMT_status.temperature <= BATTERY_TEMPERATURE_N18)
    {
        g_BatteryNotifyCode |= 0x0002;
        battery_xlog_printk(BAT_LOG_CRTI, "[Discharging] bat_temp(%d) out of range(too low)\n", BMT_status.temperature);
    }
    
    if((BMT_status.temperature <= MIN_CHARGE_TEMPERATURE) && (BMT_status.charger_exist == KAL_TRUE))
    {
        g_BatteryNotifyCode |= 0x0020;
        battery_xlog_printk(BAT_LOG_CRTI, "[Charging] bat_temp(%d) out of range(too low)\n", BMT_status.temperature);
    }
#endif
#endif
/*[805255][Temperature]MS prompts "the handset has stopped charging!" though without charger. by [bin.song.hz] at [2014.10.14] end*/
    battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] BATTERY_NOTIFY_CASE_0002_VBATTEMP (%x)\n", g_BatteryNotifyCode);
 /*[FR618922][soul45][charge]import new temperature policy by [wangxingxing] at [20140327] end*/    
#endif
}
...
static void mt_battery_thermal_check(void)
{
	if( (g_battery_thermal_throttling_flag==1) || (g_battery_thermal_throttling_flag==3) )
    {
        ...
        //ignore default rule
#else    
		/* [803383][MT6582][Soul45]Modify behavior while temperature is over 60C or under -20C by [bin.song.hz] at [2014.10.10] begin */
		if((BMT_status.temperature >= 60) || (BMT_status.temperature <= -20))
		/* [803383][MT6582][Soul45]Modify behavior while temperature is over 60C or under -20C by [bin.song.hz] at [2014.10.10] end */
        {
            #if defined(CONFIG_POWER_EXT)
            ...
            #else
            {
                ...
                else
                {
					...
					/* [803383][MT6582][Soul45]Modify behavior while temperature is over 60C or under -20C by [bin.song.hz] at [2014.10.9] begin */
                    if(g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || g_platform_boot_mode == LOW_POWER_OFF_CHARGING_BOOT)
                    {
                        return;
                    }
                    /* [803383][MT6582][Soul45]Modify behavior while temperature is over 60C or under -20C by [bin.song.hz] at [2014.10.9] end */
