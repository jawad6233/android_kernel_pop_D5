...
static struct mutex ps_mutex;//between send and recv;
...
static int boot_prox_mean=0;

/*yi.zheng.hz modify for debug p sensor calibration at 2013-06-27 start*/
#ifdef TARGET_BUILD_MMITEST  /*only open in engineer version*/
...
static int tmd2772_read_data_for_cali(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali, int cali_num)
{
	...
	for(j = 0;j<M;j++)
	 ...
	 if(i == M)
	 {
		...
		if(i == cali_num)
		{
			boot_prox_mean=sum/cali_num;//------------- llf this prox_mean should be pull off to mmi;
...
GET_THRE:
			...
			ps_data_cali->close = prox_mean+PROX_CLOSE_LIMIT_OFFSET; //this can customize, adjust close and farway distance
			ps_data_cali->far_away=prox_mean+PROX_FAR_LIMIT_OFFSET;
			ps_data_cali->valid = 1;
...
EXIT_ERR:
	...
	if((boot_ps_ok==1)&&((boot_ps_cali.close-PROX_CLOSE_LIMIT_OFFSET)>200))
	{
		databuf[0] = TMD2772_CMM_OFFSET;
...
static ssize_t show_ps_status_value(struct device_driver *ddri, char *buf)
{
	...
	len = snprintf(buf, PAGE_SIZE, "%d:%d:%d:%d\n", 
		boot_prox_mean,ps_cali.offset,ps_cali.close, ps_cali.far_away);
	return len;
}
